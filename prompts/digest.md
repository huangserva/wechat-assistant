...
# digest 看的是昨天数据，允许复用 1 小时内的最近一次成功 refresh/sync
# 如果 refresh 退出码=2，表示密钥过期（微信重启过），发告警后终止
python3 prepare_runtime.py --config /Users/serva/wechat-assistant/config.yaml --refresh-max-age-sec 3600 --sync-max-age-sec 3600
```
