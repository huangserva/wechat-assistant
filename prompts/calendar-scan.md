...
从微信私聊和工作群中扫描日程信息，创建 Apple Calendar 事件，推送到飞书。
...
```bash
python3 /Users/serva/.hermes/skills/social-media/wechat-assistant/scripts/db_writer.py --db ~/wechat-assistant/assistant.db --table calendar_events --data '[{items JSON array, 每个item含 event_date,content,contact,status}]'
...
```
---
🕐 cron: wechat-calendar-scan · 运行于 YYYY-MM-DD HH:MM · 扫描窗口 HH:MM~HH:MM · 结果：N新日程
```

即使没有新日程也发一条简短的状态消息：
```
📅 wechat-calendar-scan · YYYY-MM-DD HH:MM · 无新日程 · 待确认: N
```
