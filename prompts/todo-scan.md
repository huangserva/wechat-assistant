...
# 共享运行时准备：3 分钟内复用最近一次成功 refresh/sync，避免多个 cron 重复跑
# 如果 refresh 退出码=2，表示密钥过期（微信重启过），发告警后终止
python3 prepare_runtime.py --config /Users/serva/wechat-assistant/config.yaml --refresh-max-age-sec 180 --sync-max-age-sec 180
```
